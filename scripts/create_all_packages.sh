...
if [ "${DRYRUN}zz" == "zz" ]; then
    ...
    ssh -i ~/ssh_key ci@download.kiwix.org mkdir -p /data/download/nightly/$CURRENT_DATE
    scp -r -p -i ~/ssh_key build/* ci@download.kiwix.org:/data/download/nightly/$CURRENT_DATE
else
