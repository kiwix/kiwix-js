 ...
 function($, zimArchiveLoader, util, uiUtil, cookies, abstractFilesystemAccess, q) {
    ...
    function displayArticleContentInIframe(dirEntry, htmlArticle) {
        ...
        function loadCSSJQuery() {
            ...
            var collapsedBlocks = iframe.querySelectorAll('details:not([open]), .collapsible-block:not(.open-block), .collapsible-heading:not(.open-block)');
            ...
            for (var i = collapsedBlocks.length; i--;) {
                if (/DETAILS/.test(collapsedBlocks[i].tagName)) {
                    collapsedBlocks[i].open = true;
                } else {
