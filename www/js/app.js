...
if (
  (isServiceWorkerAvailable() ||
    (isMessageChannelAvailable() &&
      /^(moz|chrome)-extension:/i.test(window.location.protocol))) &&
...
if (!/^chrome-extension:/i.test(window.location.protocol)) {
  ...
  document.getElementById("serviceWorkerLocalDescription").style.display =
    "none";
}
...
if (window.matchMedia("(prefers-color-scheme)").media === "not all") {
  var optionsToBeRemoved = document
...
function resizeIFrame() {
  ...
  const nestedFrame =
    libraryContent.contentWindow.document.getElementById("libraryIframe");
  ...
  if (
    articleWindow.document &&
    articleWindow.document.getElementById("replay_iframe")
  ) {
    thisArticleWindow =
      articleContainer.contentWindow.document.getElementById(
        "replay_iframe",
      ).contentWindow;
  }
...
searchArticle.addEventListener("click", function () {
  ...
  if (
    appstate.search.prefix === prefix &&
    !/^(cancelled|complete)$/.test(appstate.search.status)
  )
    return;
...
document
  .getElementById("formArticleSearch")
...
function getDefaultLanguageAndTranslateApp() {
  ...
  if (!params.overrideBrowserLanguage) {
    if (/^en|es|fr$/.test(defaultBrowserLanguage.base)) {
      console.log(
        "Supported default browser language is: " +
          defaultBrowserLanguage.base +
          " (" +
          defaultBrowserLanguage.locale +
          ")",
      );
    } else {
      console.warn(
  ...
  var languageCode =
    params.overrideBrowserLanguage || defaultBrowserLanguage.base;
  translateUI.translateApp(languageCode).catch(function (err) {
    if (languageCode !== "en") {
      var message =
        "<p>We cannot load the translation strings for language code <code>" +
        languageCode +
        "</code>";
      ...
      if (err)
        message += "<p>The error message was:</p><code>" + err + "</code>";
...
document
  ...
  .addEventListener("change", function (e) {
    ...
    if (language === "other") {
      uiUtil
        .systemAlert(
          translateUI.t("dialog-other-language-message") ||
          ...
          translateUI.t("configure-language-selector-other") || "More soon...",
        )
        .then(function () {
          document.getElementById("languageSelector").value =
            params.overrideBrowserLanguage || "default";
        });
...
prefixElement.addEventListener("keydown", function (e) {
  ...
  if (/^((Arrow)?Down|(Arrow)?Up|Enter)$/.test(e.key)) {
    ...
    keyPressHandled = true;
    var activeElement =
      document.querySelector("#articleList .hover") ||
      document.querySelector("#articleList a");
    ...
    if (/Down/.test(e.key)) {
      if (activeElement.classList.contains("hover")) {
        ...
        if (!uiUtil.isElementInView(nextElement, true))
    ...
    if (/Up/.test(e.key)) {
      ...
      var previousElement =
        activeElement.previousElementSibling || activeElement;
      if (!uiUtil.isElementInView(previousElement, true))
...
document
  .getElementById("btnRandomArticle")
...
document
  .getElementById("btnRescanDeviceStorage")
...
document.getElementById("btnHome").addEventListener("click", function (event) {
  ...
  var articleListHeaderMessage = document.getElementById(
    "articleListHeaderMessage",
  );
  while (articleList.firstChild)
    articleList.removeChild(articleList.firstChild);
  while (articleListHeaderMessage.firstChild)
    articleListHeaderMessage.removeChild(articleListHeaderMessage.firstChild);
  uiUtil.spinnerDisplay(false);
  // document.getElementById('articleContent').style.display = 'none';
  // Empty and purge the article contents
  var articleContent = document.getElementById("articleContent");
  var articleContentDoc = articleContent
    ? articleContent.contentDocument
    : null;
  while (articleContentDoc.firstChild)
    articleContentDoc.removeChild(articleContentDoc.firstChild);
  if (selectedArchive !== null && selectedArchive.isReady()) {
    document.getElementById("welcomeText").style.display = "none";
    goToMainArticle();
  }
...
document
  .querySelectorAll('input[name="contentInjectionMode"][type="radio"]')
...
document
  .getElementById("useCanvasElementsCheck")
  .addEventListener("change", function () {
    if (this.checked) {
      ...
      settingsStore.setItem(
        "useCanvasElementsForWebpTranscoding",
        false,
        Infinity,
      );
    }
...
document.getElementById("btnReset").addEventListener("click", function () {
  uiUtil
    .systemAlert(
      translateUI.t("dialog-reset-warning-message") ||
        "This will reset the app to a freshly installed state, deleting all app caches and settings!",
      translateUI.t("dialog-reset-warning-title") || "WARNING!",
      true,
    )
...
document
  .getElementById("bypassAppCacheCheck")
  .addEventListener("change", function () {
    if (params.contentInjectionMode !== "serviceworker") {
      uiUtil.systemAlert(
        translateUI.t("dialog-bypassappcachecheck-message") ||
          "This setting can only be used in ServiceWorker mode!",
      );
      this.checked = false;
    } else {
...
document
  .getElementById("disableDragAndDropCheck")
  .addEventListener("change", function () {
    params.disableDragAndDrop = !!this.checked;
    settingsStore.setItem(
      "disableDragAndDrop",
      params.disableDragAndDrop,
      Infinity,
    );
    uiUtil
      .systemAlert(
        translateUI.t("dialog-disabledragdrop-message") ||
          "<p>We will now attempt to reload the app to apply the new setting.</p>" +
            "<p>(If you cancel, then the setting will only be applied when you next start the app.)</p>",
        translateUI.t("dialog-disabledragdrop-title") || "Reload app",
        true,
      )
...
document
  .querySelectorAll('input[type="checkbox"][name=hideActiveContentWarning]')
  .forEach(function (element) {
    element.addEventListener("change", function () {
      params.hideActiveContentWarning = !!this.checked;
      settingsStore.setItem(
        "hideActiveContentWarning",
        params.hideActiveContentWarning,
        Infinity,
      );
    });
  });
document
  .getElementById("slideAwayCheck")
  .addEventListener("change", function (e) {
    ...
    if (typeof navigator.getDeviceStorages === "function") {
      ...
      uiUtil.systemAlert(
        translateUI.t("dialog-slideawaycheck-message") ||
          "This setting may not work correctly on Firefox OS. " +
            "If you find that some ZIM links become unresponsive, try turning this setting off.",
...
document
  .querySelectorAll('input[type="checkbox"][name=showUIAnimations]')
  .forEach(function (element) {
    element.addEventListener("change", function () {
      params.showUIAnimations = !!this.checked;
      settingsStore.setItem(
        "showUIAnimations",
        params.showUIAnimations,
        Infinity,
      );
    });
  });
document
  .getElementById("useHomeKeyToFocusSearchBarCheck")
  .addEventListener("change", function (e) {
    params.useHomeKeyToFocusSearchBar = e.target.checked;
    settingsStore.setItem(
      "useHomeKeyToFocusSearchBar",
      params.useHomeKeyToFocusSearchBar,
      Infinity,
    );
    switchHomeKeyToFocusSearchBar();
    if (params.useHomeKeyToFocusSearchBar && params.slideAway) {
      uiUtil.systemAlert(
        translateUI.t("dialog-focussearchbarcheck-message") ||
          "Please note that this setting focuses the search bar when you go to a ZIM landing page, disabling sliding away of header and footer on that page (only).",
        translateUI.t("dialog-warning") || "Warning",
      );
    }
  });
document
  .querySelectorAll('input[type="checkbox"][name=openExternalLinksInNewTabs]')
  .forEach(function (element) {
    element.addEventListener("change", function () {
...
document
  .getElementById("cachedAssetsModeRadioTrue")
  .addEventListener("change", function (e) {
    if (e.target.checked) {
...
document
  .getElementById("cachedAssetsModeRadioFalse")
  .addEventListener("change", function (e) {
    if (e.target.checked) {
...
document
  .getElementById("titleSearchRange")
  .addEventListener("change", function (e) {
...
document.getElementById("kiwixLogo").ondragstart = function () {
  return false;
};
document.getElementById("navigationButtons").ondragstart = function () {
  return false;
};
...
function switchHomeKeyToFocusSearchBar() {
  var iframeContentWindow =
    document.getElementById("articleContent").contentWindow;
...
function checkAndDisplayInjectionModeChangeAlert() {
  ...
  if (
    ...
    message = [
      translateUI.t("dialog-serviceworker-defaultmodechange-message") ||
    ...
    message = [
      translateUI.t("dialog-serviceworker-unsupported-message") ||
      ...
      translateUI.t("dialog-serviceworker-unsupported-title") ||
    ...
    uiUtil
      .systemAlert(
        ...
        true,
        null,
        translateUI.t("dialog-ok") || "Okay",
      )
      .then(function (result) {
        if (result) {
          // If user selected OK, then do not display again ever
          settingsStore.setItem(
            "defaultModeChangeAlertDisplayed",
            true,
            Infinity,
          );
        }
...
function refreshAPIStatus() {
  ...
  setTimeout(function () {
    ...
    apiStatusPanel.classList.remove(
      "card-success",
      "card-warning",
      "card-danger",
    );
    ...
    if (isMessageChannelAvailable()) {
      messageChannelStatus.textContent =
        translateUI.t("api-messagechannel-available") ||
        "MessageChannel API available";
      ...
      messageChannelStatus.textContent =
        translateUI.t("api-messagechannel-unavailable") ||
        "MessageChannel API unavailable";
      messageChannelStatus.classList.remove("apiAvailable", "apiUnavailable");
      messageChannelStatus.classList.add("apiUnavailable");
    }
    if (isServiceWorkerAvailable()) {
      if (isServiceWorkerReady()) {
        serviceWorkerStatus.textContent =
          translateUI.t("api-serviceworker-available-registered") ||
          "ServiceWorker API available, and registered";
        serviceWorkerStatus.classList.remove("apiAvailable", "apiUnavailable");
        serviceWorkerStatus.classList.add("apiAvailable");
      } else {
        apiPanelClass = "card-warning";
        serviceWorkerStatus.textContent =
          translateUI.t("api-serviceworker-available-unregistered") ||
          "ServiceWorker API available, but not registered";
        serviceWorkerStatus.classList.remove("apiAvailable", "apiUnavailable");
      ...
      apiPanelClass = "card-warning";
      serviceWorkerStatus.textContent =
        translateUI.t("api-serviceworker-unavailable") ||
    ...
    var apiName =
      params.storeType === "cookie"
        ? translateUI.t("api-cookie") || "Cookie"
        : params.storeType === "local_storage"
          ? translateUI.t("api-localstorage") || "Local Storage"
          : translateUI.t("api-none") || "None";
    settingsStoreStatusDiv.textContent =
      (translateUI.t("api-storage-used-label") ||
        "Settings Storage API in use:") +
      " " +
      apiName;
    settingsStoreStatusDiv.classList.remove("apiAvailable", "apiUnavailable");
    settingsStoreStatusDiv.classList.add(
      params.storeType === "none" ? "apiUnavailable" : "apiAvailable",
    );
    apiPanelClass =
      params.storeType === "none" ? "card-warning" : apiPanelClass;
    ...
    apiPanelClass = params.decompressorAPI.errorStatus
      ? "card-danger"
      : apiName === "WASM"
        ? apiPanelClass
        : "card-warning";
    decompAPIStatusDiv.className = apiName
      ? params.decompressorAPI.errorStatus
        ? "apiBroken"
        : apiName === "WASM"
          ? "apiAvailable"
          : "apiSuboptimal"
    ...
    if (apiName && params.decompressorAPI.decompressorLastUsed) {
      apiName +=
        " [&nbsp;" + params.decompressorAPI.decompressorLastUsed + "&nbsp;]";
    }
    apiName =
      params.decompressorAPI.errorStatus ||
      apiName ||
      translateUI.t("api-error-uninitialized_feminine") ||
      "Not initialized";
    // innerHTML is used here because the API name may contain HTML entities like &nbsp;
    decompAPIStatusDiv.innerHTML =
      (translateUI.t("api-decompressor-label") || "Decompressor API:") +
      " " +
      apiName;
    ...
    pwaOriginStatusDiv.innerHTML =
      (translateUI.t("api-pwa-origin-label") || "PWA Origin:") +
      " " +
    ...
    document.getElementById("bypassAppCacheDiv").style.display =
      params.contentInjectionMode === "serviceworker" ? "block" : "none";
    // Check to see whether we need to alert the user that we have switched to ServiceWorker mode by default
    if (!params.defaultModeChangeAlertDisplayed)
      ...
      channel.port1.onmessage = function (event) {
        ...
        if (cache.error) {
          ...
          if (
            cache.type === "cacheAPI" &&
            selectedArchive &&
            selectedArchive.zimType === "zimit" &&
            appstate.isReplayWorkerAvailable
          ) {
...
function refreshCacheStatus() {
  ...
  document.getElementById("jqueryCompatibility").style.display =
    params.contentInjectionMode === "jquery" ? "" : "none";
  ...
  getAssetsCacheAttributes().then(function (cache) {
    if (cache.type === "cacheAPI" && ASSETS_CACHE !== cache.name) {
      console.error(
        "DEV: The ASSETS_CACHE defined in app.js does not match the ASSETS_CACHE defined in service-worker.js!",
      );
    }
...
function initServiceWorkerMessaging() {
  if (!(isServiceWorkerAvailable() && isMessageChannelAvailable())) {
    console.warn(
      "Cannot initiate ServiceWorker messaging, because one or more API is unavailable!",
    );
    return;
  }
  ...
  navigator.serviceWorker.onmessage = function (event) {
    if (event.data.error) {
      ...
      const videoException =
        selectedArchive.zimType === "zimit" &&
        /\/\/youtubei.*player/.test(event.data.title);
      ...
      if (
        event.data.zimFileName !== selectedArchive.file.name &&
        !videoException
      ) {
        ...
        if (videoException) {
          ...
          console.warn(
            ">>> Allowing passthrough of SW request to process Zimit video <<<",
          );
        }
  ...
  if (navigator.serviceWorker.controller) {
    ...
    console.warn(
      "The Service Worker needs more time to load, or else the app was force-refreshed...",
    );
    serviceWorkerRegistration = null;
    setTimeout(initServiceWorkerMessaging, 1600);
  } else if (params.contentInjectionMode === "serviceworker") {
    console.error(
      "The Service Worker is not controlling the current page! We have to reload.",
    );
    ...
    if (!params.noPrompts) {
      uiUtil
        .systemAlert(
          "<p>No Service Worker is registered, meaning this app will not currently work offline!</p><p>Would you like to switch to ServiceWorker mode?</p>",
          "Offline use is disabled!",
          true,
        )
        .then(function (response) {
          if (response) {
...
function setContentInjectionMode(value) {
  ...
  params.oldInjectionMode = params.serviceWorkerLocal
    ? "serviceworkerlocal"
    : params.contentInjectionMode;
  ...
  if (value === "jquery") {
    if (!params.appCache) {
      uiUtil
        .systemAlert(
          translateUI.t("dialog-bypassappcache-conflict-message") ||
            'You must deselect the "Bypass AppCache" option before switching to JQuery mode!',
          translateUI.t("dialog-bypassappcache-conflict-title") ||
            'Deselect "Bypass AppCache"',
        )
        .then(function () {
          setContentInjectionMode("serviceworker");
        });
    ...
    if (params.referrerExtensionURL) {
      ...
      message =
        translateUI.t("dialog-launchlocal-message") ||
        "This will switch to using locally packaged code only. Some configuration settings may be lost.<br/><br/>" +
      ...
      var launchLocal = function () {
        ...
        var uriParams =
          "?allowInternetAccess=false&contentInjectionMode=jquery&hideActiveContentWarning=false";
        uriParams += "&appTheme=" + params.appTheme;
        uriParams += "&showUIAnimations=" + params.showUIAnimations;
        window.location.href =
          params.referrerExtensionURL + "/www/index.html" + uriParams;
    ...
    if (
      /^(moz|chrome)-extension:/.test(protocol) &&
      !params.serviceWorkerLocal
    ) {
      launchBrowserExtensionServiceWorker();
    } else {
      if (!isServiceWorkerAvailable()) {
        message =
          translateUI.t("dialog-launchpwa-unsupported-message") ||
          "<p>Unfortunately, your browser does not appear to support ServiceWorker mode, which is now the default for this app.</p>" +
            "<p>You can continue to use the app in the (now deprecated) JQuery mode, but note that this mode only works well with " +
            "ZIM archives that have static content, such as Wikipedia / Wikimedia ZIMs or Stackexchange.</p>" +
            "<p>If you can, we recommend that you update your browser to a version that supports ServiceWorker mode.</p>";
        if (!params.noPrompts) {
          uiUtil
            .systemAlert(
              message,
              translateUI.t("dialog-launchpwa-unsupported-title") ||
                "ServiceWorker API not available",
              true,
              null,
              translateUI.t("dialog-serviceworker-unsupported-fallback") ||
                "Use JQuery mode",
            )
            .then(function (response) {
              if (params.referrerExtensionURL && response) {
                var uriParams =
                  "?allowInternetAccess=false&contentInjectionMode=jquery&defaultModeChangeAlertDisplayed=true";
                window.location.href =
                  params.referrerExtensionURL + "/www/index.html" + uriParams;
              } else {
      ...
      if (!isMessageChannelAvailable()) {
        uiUtil
          .systemAlert(
            translateUI.t("dialog-messagechannel-unsupported-message") ||
              "The MessageChannel API is not available on your device. Falling back to JQuery mode...",
            translateUI.t("dialog-messagechannel-unsupported-title") ||
      ...
      if (!isServiceWorkerReady()) {
        var serviceWorkerStatus = document.getElementById(
          "serviceWorkerStatus",
        );
        serviceWorkerStatus.textContent =
          "ServiceWorker API available : trying to register it...";
        if (navigator.serviceWorker.controller) {
          ...
          navigator.serviceWorker
            ...
            .catch(function (err) {
              if (protocol === "moz-extension:") {
                ...
                var message =
                  (translateUI.t(
                    "dialog-serviceworker-registration-failure-message",
                  ) ||
                    "The Service Worker could not be properly registered. Switching back to JQuery mode... Error message:") +
                  " " +
                  err;
                if (protocol === "file:") {
                  message +=
                ...
                if (!params.noPrompts) {
                  uiUtil
                    .systemAlert(
                      message,
                      translateUI.t(
                        "dialog-serviceworker-registration-failure-title",
  ...
  $("input:radio[name=contentInjectionMode]")
    .filter('[value="' + trueMode + '"]')
    .prop("checked", true);
...
function launchBrowserExtensionServiceWorker() {
  ...
  message +=
    (translateUI.t("dialog-allow-internetaccess-message4") ||
      "need one-time access to our secure server so that the app can re-launch as a Progressive Web App (PWA). " +
        ...
        "<p>WARNING: This will attempt to access the following server:<br/>") +
    params.PWAServer +
    "</p>";
  var launchPWA = function () {
    uiUtil.spinnerDisplay(false);
    var uriParams =
      "?contentInjectionMode=serviceworker&allowInternetAccess=true";
    uriParams +=
      "&referrerExtensionURL=" +
      encodeURIComponent(
        window.location.href.replace(/\/www\/index.html.*$/i, ""),
      );
  ...
  var checkPWAIsOnline = function () {
    uiUtil.spinnerDisplay(
      true,
      translateUI.t("dialog-serveraccess-check") || "Checking server access...",
    );
    uiUtil.checkServerIsAccessible(
      params.PWAServer + "www/img/icons/kiwix-32.png",
      launchPWA,
      function () {
        uiUtil.spinnerDisplay(false);
        uiUtil
          .systemAlert(
            translateUI.t("dialog-serveraccess-check-failed") ||
              "The server is not currently accessible! " +
                "<br/><br/>(Kiwix needs one-time access to the server to cache the PWA)." +
                "<br/>Please try again when you have a stable Internet connection.",
            translateUI.t("dialog-error-title") || "Error!",
          )
          .then(function () {
            settingsStore.setItem("allowInternetAccess", false, Infinity);
            setContentInjectionMode(params.oldInjectionMode || "jquery");
  ...
  if (settingsStore.getItem("allowInternetAccess") === "true") {
    if (PWASuccessfullyLaunched) {
      ...
      uiUtil
        .systemAlert(
          translateUI.t("dialog-launchpwa-fail-message") ||
            "The last attempt to launch the PWA appears to have failed.<br/><br/>Do you wish to try again?",
          translateUI.t("dialog-launchpwa-fail-title") ||
            "Confirmation to retry PWA launch",
          true,
        )
        .then(function (response) {
          if (response) {
    ...
    uiUtil
      .systemAlert(
        message,
        translateUI.t("dialog-allow-internetaccess-title") ||
      ...
      .then(function (response) {
        if (response) {
          ...
          params.oldInjectionMode =
            params.oldInjectionMode === "serviceworker"
              ? /^chrome-extension:/i.test(window.location.protocol)
                ? "serviceworkerlocal"
                : null
              : params.oldInjectionMode;
          setContentInjectionMode(params.oldInjectionMode || "jquery");
...
function searchForArchivesInPreferencesOrStorage() {
  ...
  if (
    listOfArchivesFromSettingsStore !== null &&
    listOfArchivesFromSettingsStore !== undefined &&
...
function populateDropDownListOfArchives(archiveDirectories) {
  ...
  for (var i = 0; i < archiveDirectories.length; i++) {
    ...
    if (archiveDirectory === "/") {
      uiUtil.systemAlert(
        translateUI.t("dialog-invalid-archivelocation-message") ||
        ...
        translateUI.t("dialog-invalid-archivelocation-title") ||
          "Error: invalid archive files location",
      );
    } else {
      comboArchiveList.options[i] = new Option(
        archiveDirectory,
        archiveDirectory,
      );
    }
  ...
  document
    ...
    .addEventListener("change", setLocalArchiveFromArchiveList);
  if (comboArchiveList.options.length > 0) {
    var lastSelectedArchive = settingsStore.getItem("lastSelectedArchive");
    if (
      lastSelectedArchive !== null &&
      lastSelectedArchive !== undefined &&
      lastSelectedArchive !== ""
    ) {
      // Attempt to select the corresponding item in the list, if it exists
      if (
        $("#archiveList option[value='" + lastSelectedArchive + "']").length > 0
      ) {
    ...
    uiUtil
      .systemAlert(
        translateUI.t("dialog-welcome-message") ||
          "Welcome to Kiwix! This application needs at least a ZIM file in your SD-card (or internal storage). Please download one and put it on the device (see About section). Also check that your device is not connected to a computer through USB device storage (which often locks the SD-card content)",
        translateUI.t("dialog-welcome-title") || "Welcome",
      )
      .then(function () {
        document.getElementById("btnAbout").click();
  ...
  if (archiveDirectory && archiveDirectory.length > 0) {
    ...
    if (regexpResults && regexpResults.length > 0) {
      ...
      if (selectedStorage === null) {
        uiUtil.systemAlert(
          (translateUI.t("dialog-devicestorage-error-message") ||
            "Unable to find which device storage corresponds to directory") +
            " " +
            archiveDirectory,
          "Error: no matching storage",
        );
      }
    } else {
      ...
      if (storages.length === 1) {
        ...
        uiUtil.systemAlert(
          "Something weird happened with the DeviceStorage API: found a directory without prefix:" +
            " " +
            archiveDirectory +
            ", " +
            "but there were" +
          ...
          "Error: unprefixed directory",
        );
      }
    }
    settingsStore.setItem("lastSelectedArchive", archiveDirectory, Infinity);
    zimArchiveLoader.loadArchiveFromDeviceStorage(
      selectedStorage,
      archiveDirectory,
...
function displayFileSelect() {
  const isFireFoxOsNativeFileApiAvailable =
  ...
  if (
    ...
    isPlatformMobilePhone = true;

  console.debug(
    `File system api is ${
      params.isFileSystemApiSupported ? "" : "not "
    }supported`,
  );
  console.debug(
    `Webkit directory api ${
      params.isWebkitDirApiSupported ? "" : "not "
    }supported`,
  );
  console.debug(
    `Firefox os native file ${
      isFireFoxOsNativeFileApiAvailable ? "" : "not "
    }support api`,
  );

  document.getElementById("openLocalFiles").style.display = "block";
  if (
    (params.isFileSystemApiSupported || params.isWebkitDirApiSupported) &&
    !isPlatformMobilePhone
  ) {
    document.getElementById("chooseArchiveFromLocalStorage").style.display = "";
  ...
  if (!params.disableDragAndDrop) {
    ...
    globalDropZone.addEventListener("dragover", function (e) {
      e.preventDefault();
      if (configDropZone.style.display === "none")
        document.getElementById("btnConfigure").click();
  ...
  document
    ...
    .addEventListener("change", function (e) {
      ...
      localStorage.setItem(params.keyPrefix + "previousZimFileName",e.target.value,);
      if (params.isFileSystemApiSupported) {
        return abstractFilesystemAccess
          .getSelectedZimFromCache(e.target.value)
          .then(function (files) {
          ...
          .catch(function (err) {
            ...
            return uiUtil.systemAlert(
              translateUI.t("dialog-fielhandle-fail-message") ||
                "We were unable to retrieve a file handle for the selected archive. Please pick the file or folder again.",
              translateUI.t("dialog-fielhandle-fail-title") ||
                "Error retrieving archive",
            );
          });
      } else {
        if (webKitFileList === null) {
          const element =
            localStorage.getItem(keyPrefix + "zimFilenames").split("|").length === 1
              ? "archiveFiles"
              : "archiveFolders";
        ...
        const files = abstractFilesystemAccess.getSelectedZimFromWebkitList(
          webKitFileList,
          e.target.value,
        );
        setLocalArchiveFromFileList(files);
  ...
  if (params.isFileSystemApiSupported) {
    ...
    folderSelect.addEventListener("click", async function (e) {
      e.preventDefault();
      const previousZimFiles =
        await abstractFilesystemAccess.selectDirectoryFromPickerViaFileSystemApi();
      if (previousZimFiles.length !== 0)
        setLocalArchiveFromFileList(previousZimFiles);
    });
  }
  if (params.isWebkitDirApiSupported) {
    ...
    folderSelect.addEventListener("change", async function (e) {
      ...
      const lastFilename = localStorage.getItem(params.keyPrefix + "previousZimFileName") ?? "";
      ...
      for (const file of e.target.files) {
        ...
        if (regex.test(file.name) || file.name === lastFilename)
      ...
      if (previousZimFile.length !== 0)
        setLocalArchiveFromFileList(previousZimFile);
      await abstractFilesystemAccess.updateZimDropdownOptions(
        filenames,
        previousZimFile.length !== 0 ? lastFilename : "",
      );
    });
  }
  ...
  if (params.isFileSystemApiSupported) {
    ...
    archiveFiles.addEventListener("click", async function (e) {
      e.preventDefault();
      const files =
        await abstractFilesystemAccess.selectFileFromPickerViaFileSystemApi(e);
      setLocalArchiveFromFileList(files);
    });
  } else {
...
function useLegacyFilePicker() {
  ...
  archiveFiles.addEventListener("change", async function (e) {
    if (params.isWebkitDirApiSupported || params.isFileSystemApiSupported) {
      ...
      localStorage.setItem(params.keyPrefix + "zimFilenames",[activeFilename].join("|"),);
      await abstractFilesystemAccess.updateZimDropdownOptions(
        [activeFilename],
        activeFilename,
      );
...
document
  .getElementById("archiveFilesLbl")
...
async function handleFileDrop(packet) {
  ...
  document.getElementById("fileSelectionButtonContainer").style.display =
    "none";
  ...
  if (params.isFileSystemApiSupported)
    loadZim =
      await abstractFilesystemAccess.handleFolderOrFileDropViaFileSystemAPI(
        packet,
      );
  else if (params.isWebkitDirApiSupported) {
    const ret =
      await abstractFilesystemAccess.handleFolderOrFileDropViaWebkit(packet);
    loadZim = ret.loadZim;
...
document
  .getElementById("selectorsDisplayLink")
...
function setLocalArchiveFromFileList(files) {
  ...
  for (var i = files.length; i--; ) {
    ...
    if (!/\.(?:zim\w{0,2})$/i.test(files[i].name)) {
      uiUtil.systemAlert(
        translateUI.t("dialog-invalid-zim-message") ||
          "One or more files does not appear to be a ZIM file!",
        translateUI.t("dialog-invalid-zim-title") || "Invalid file format",
      );
      return;
    }
  }
  zimArchiveLoader.loadArchiveFromFiles(
    files,
    archiveReadyCallback,
    function (message, label) {
      // callbackError which is called in case of an error
    ...
    request.onreadystatechange = function () {
      if (request.readyState === XMLHttpRequest.DONE) {
        if (
          ...
          reject(
            new Error("HTTP status " + request.status + " when reading " + url),
          );
        }
      }
    };
...
function searchDirEntriesFromPrefix(prefix) {
  if (selectedArchive !== null && selectedArchive.isReady()) {
    ...
    appstate.search = {
      ...
      status: "init",
      type: "",
      size: params.maxSearchResultsSize,
    };
    var activeContent = document.getElementById("activeContent");
    if (activeContent) activeContent.style.display = "none";
    selectedArchive.findDirEntriesWithPrefix(
      appstate.search,
    ...
    uiUtil
      .systemAlert(
        translateUI.t("dialog-archive-notset-message") ||
          "Archive not set: please select an archive",
        translateUI.t("dialog-archive-notset-title") || "No archive selected",
      )
      .then(function () {
        document.getElementById("btnConfigure").click();
      });
...
function populateListOfArticles(dirEntryArray, reportingSearch) {
  ...
  var articleListHeaderMessageDiv = document.getElementById(
    "articleListHeaderMessage",
  );
  ...
  if (stillSearching) {
    message =
      "Searching [" + reportingSearch.type + "]... found: " + nbDirEntry;
  } else if (nbDirEntry >= params.maxSearchResultsSize) {
    message =
      "First " +
      params.maxSearchResultsSize +
      " articles found (refine your search).";
  } else {
    message =
      "Finished. " +
      (nbDirEntry || "No") +
      " articles found" +
      (reportingSearch.type === "basic"
        ? ": try fewer words for full search."
        : ".");
  }
  ...
  var listLength =
    dirEntryArray.length < params.maxSearchResultsSize
      ? dirEntryArray.length
      : params.maxSearchResultsSize;
...
function findDirEntryFromDirEntryIdAndLaunchArticleRead(dirEntryId) {
  if (selectedArchive.isReady()) {
    ...
    uiUtil.systemAlert(
      translateUI.t("dialog-file-notset-message") || "Data files not set",
...
function readArticle(dirEntry) {
  if (dirEntry === null || dirEntry === undefined) {
    console.error(
      "The directory entry for the requested article was not found (null or undefined)",
    );
    uiUtil.spinnerDisplay(false);
    return;
  }
  ...
  uiUtil.spinnerDisplay(
    true,
    (translateUI.t("spinner-loading") || "Loading") + " " + message,
  );
  ...
  if (params.contentInjectionMode === "serviceworker") {
    ...
    var encodedUrl = dirEntry.url.replace(
      /[^/]+/g,
    ...
    if (
      ...
      zimitMessageChannel.port1.onmessage = function (event) {
        if (event.data.error) {
          console.error(
            "Reading Zimit archives in ServiceWorker mode is not supported in this browser",
            event.data.error,
          );
          return handleUnsupportedReplayWorker(dirEntry);
      ...
      selectedArchive.readUtf8File(dirEntry, function (fileDirEntry, content) {
        ...
        if (fileDirEntry.zimitRedirect) {
          ...
          return selectedArchive
            .getDirEntryByPath(fileDirEntry.zimitRedirect)
            .then(readArticle);
        } else {
...
function articleLoadedSW(iframeArticleContent) {
  ...
  document.getElementById("cachingAssets").textContent =
    translateUI.t("spinner-caching-assets") || "Caching assets...";
  ...
  if (!params.disableDragAndDrop) {
    var doc = iframeArticleContent.contentDocument
      ? iframeArticleContent.contentDocument.documentElement
      : null;
  ...
  if (iframeArticleContent.contentWindow) {
    ...
    if (params.useHomeKeyToFocusSearchBar) {
      iframeArticleContent.contentWindow.onkeydown = focusPrefixOnHomeKey;
    }
    ...
    iframeArticleContent.contentWindow.onunload = function () {
      ...
      while (articleList.firstChild)
        articleList.removeChild(articleList.firstChild);
      while (articleListHeaderMessage.firstChild)
        articleListHeaderMessage.removeChild(
          articleListHeaderMessage.firstChild,
        );
      document.getElementById("articleListWithHeader").style.display = "none";
      document.getElementById("prefix").value = "";
    };
...
function handleClickOnReplayLink(ev, anchor) {
  ...
  var containingDocDomainPath =
    anchor.ownerDocument.location.hostname +
    anchor.ownerDocument.location.pathname;
  ...
  if (
    anchor.protocol.replace(/s:/, ":") !==
      document.location.protocol.replace(/s:/, ":") ||
    pseudoDomainPath === containingDocDomainPath ||
    ev.ctrlKey ||
    ev.metaKey ||
    ev.button === 1
  )
  ...
  if (zimUrl) {
    ...
    selectedArchive.getDirEntryByPath(zimUrl).then(function (dirEntry) {
      if (dirEntry) {
        var pathToArticleDocumentRoot = document.location.href.replace(
          /www\/index\.html.*$/,
          selectedArchive.file.name + "/",
        );
...
function handleUnsupportedReplayWorker(unhandledDirEntry) {
  ...
  return uiUtil.systemAlert(
    translateUI.t("dialog-unsupported-archivetype-message") ||
      "<p>You are attempting to open a Zimit-style archive, " +
    ...
    translateUI.t("dialog-unsupported-archivetype-title") ||
  ...
  var readFile = function (dirEntry) {
    if (dirEntry === null) {
      ...
      messagePort.postMessage({
        ...
        title: title,
        content: null,
        zimType: selectedArchive.zimType,
      });
    } else if (dirEntry.isRedirect()) {
      selectedArchive.resolveRedirect(dirEntry, function (resolvedDirEntry) {
        var redirectURL =
          resolvedDirEntry.namespace + "/" + resolvedDirEntry.url;
      ...
      selectedArchive.readBinaryFile(
        dirEntry,
        function (fileDirEntry, content) {
          var mimetype = fileDirEntry.getMimetype();
          // Show the spinner
          var shortTitle = dirEntry
            .getTitleOrUrl()
            .replace(/^.*?([^/]{3,18})[^/]*\/?$/, "$1 ...");
          if (
            !/moved/i.test(shortTitle) &&
            !/image|javascript|warc-headers|jsonp?/.test(mimetype)
          ) {
            uiUtil.spinnerDisplay(
              true,
              (translateUI.t("spinner-loading") || "Loading") +
                " " +
                shortTitle,
            );
...
var regexpTagsWithZimUrl =
  /(<(?:img|script|link|track)\b[^>]*?\s)(?:src|href)(\s*=\s*(["']))(?![a-z][a-z0-9+.-]+:)(.+?)(\?.*?)?(?=\3|#)/gi;
...
var regexpActiveContent =
  /<script\b(?:(?![^>]+src\b)|(?=[^>]*type=["']module["'])|(?=[^>]+src\b=["'][^"']*?\b(?:app|init|l1[08]9)\.js))(?![^<]+(?:importScript\(\)|toggleOpenSection|articleId\s?=\s?['"]|window.NREUM))(?![^>]+type\s*=\s*["'](?:math\/|[^"']*?math))/i;
...
function displayArticleContentInIframe(dirEntry, htmlArticle) {
  ...
  if (selectedArchive.zimType === "zimit") {
    var rootDirectory =
      dirEntry.url === dirEntry.url.replace(/^((?:A\/)?[^/]+\/?).*/, "$1");
    ...
    zimitPrefix = zimitPrefix
      ? zimitPrefix[1]
      : selectedArchive.zimitPrefix.replace(/^\w\/([^/]+).*/, "$1");
    zimitPrefix = (dirEntry.namespace === "C" ? "A/" : "") + zimitPrefix;
    htmlArticle = htmlArticle.replace(
      regexpZimitHtmlLinks,
      function (match, blockStart, equals, quote, relAssetUrl, blockClose) {
        var newBlock = match;
        var assetUrl = relAssetUrl;
        // console.log('Asset URL: ' + assetUrl);
        // Remove google analytics and other analytics files that cause stall
        if (
          /analytics|typepad.*stats|googleads|doubleclick|syndication/i.test(
            assetUrl,
          )
        )
          return "";
  ...
  htmlArticle = htmlArticle.replace(
    regexpTagsWithZimUrl,
    function (match, blockStart, equals, quote, relAssetUrl, querystring) {
  ...
  if (webpMachine)
    htmlArticle = htmlArticle.replace(
      /(<img\b[^>]*?\s)src(\s*=\s*["'])(?=data:image\/webp)([^"']+)/gi,
      "$1data-kiwixurl$2$3",
    );
  ...
  iframeArticleContent.onload = function () {
    ...
    if (!iframeContentDocument && window.location.protocol === "file:") {
      uiUtil.systemAlert(
        translateUI.t("dialog-blocked-fileprotocol") ||
  ...
  function parseAnchorsJQuery() {
    ...
    var escapedUrl = encodeURIComponent(dirEntry.url).replace(
      /([\\$^.|?*+/()[{])/g,
      "\\$1",
    );
    ...
    var regexpLocalAnchorHref = new RegExp(
      "^(?:#|" + escapedUrl + "#)([^#;]*$)",
    );
    var iframe = iframeArticleContent.contentDocument;
    Array.prototype.slice
      .call(iframe.querySelectorAll("a, area"))
      .forEach(function (anchor) {
        ...
        if (
          href === null ||
          href === undefined ||
          /^javascript:/i.test(anchor.protocol)
        )
          return;
        ...
        if (
          (anchor.protocol !== currentProtocol ||
          ...
          if (selectedArchive.zimType === "zimit") {
            ...
            var zimitDomain = selectedArchive.zimitPrefix.replace(
              /^\w\/([^/]+).*/,
              "$1",
            );
            newHref = href.replace(
              anchor.protocol + "//" + zimitDomain + "/",
              "",
            );
          }
          if (newHref === href) {
            ...
            anchor.addEventListener("click", function (event) {
              ...
              var clickedAnchor = uiUtil.closestAnchorEnclosingElement(
                event.target,
              );
              uiUtil.warnAndOpenExternalLinkInNewTab(event, clickedAnchor);
            });
            ...
            href =
              dirEntry.namespace + "/" + selectedArchive.zimitPrefix + newHref;
          }
        }
        ...
        var isDownloadableLink =
          anchor.hasAttribute("download") || regexpDownloadLinks.test(href);
        if (isDownloadableLink) {
          ...
          downloadAttrValue =
            /^(download|true|\s*)$/i.test(downloadAttrValue) ||
            downloadAttrValue ||
            true;
        ...
        anchor.addEventListener("click", function (e) {
          ...
          var zimUrl;
          if (
            selectedArchive.zimitPrefix &&
            ~href.indexOf(
              dirEntry.namespace + "/" + selectedArchive.zimitPrefix,
            )
          ) {
  ...
  function loadImagesJQuery() {
    ...
    var images = Array.prototype.slice.call(
      iframeArticleContent.contentDocument.querySelectorAll(
        "img[data-kiwixurl]",
      ),
    );
    ...
    (function extractImage() {
      ...
      selectedArchive
        .getDirEntryByPath(url)
        .then(function (dirEntry) {
          selectedArchive.readBinaryFile(
            dirEntry,
            function (fileDirEntry, content) {
              var mimetype = dirEntry.getMimetype();
              uiUtil.feedNodeWithDataURI(
                image,
                "src",
                content,
                mimetype,
                function () {
                  ...
                  if (srcsetArr.length) {
                    ...
                    srcsetArr.forEach(function (imgAndResolutionUrl) {
                      ...
                      var urlMatch = imgAndResolutionUrl.match(
                        /^\s*([^\s]+)\s+([0-9.]+\w+)\s*$/,
                      );
                      ...
                      selectedArchive
                        .getDirEntryByPath(url)
                        .then(function (srcEntry) {
                          selectedArchive.readBinaryFile(
                            srcEntry,
                            function (fileDirEntry, content) {
                              var mimetype = srcEntry.getMimetype();
                              uiUtil
                                .getDataUriFromUint8Array(content, mimetype)
                                ...
                                .catch(function (e) {
                                  console.error(
                                    "Could not get dataUri for image:" + url,
                                    e,
                                  );
                                  images.busy = false;
                        ...
                        .catch(function (e) {
                          console.error(
                            "Could not find DirEntry for image:" + url,
                            e,
                          );
                          images.busy = false;
                          if (srcsetCount === 0) extractImage();
  ...
  function loadCSSJQuery() {
    ...
    var iframe = iframeArticleContent.contentDocument;
    var collapsedBlocks = iframe.querySelectorAll(
      ".collapsible-block:not(.open-block), .collapsible-heading:not(.open-block)",
    );
    ...
    var cssFulfilled = 0;
    Array.prototype.slice
      .call(iframe.querySelectorAll("link[data-kiwixurl]"))
      .forEach(function (link) {
        ...
        if (selectedArchive.cssCache.has(url)) {
          ...
          if (params.assetsCache)
            document.getElementById("cachingAssets").style.display = "";
          selectedArchive
            .getDirEntryByPath(url)
            .then(function (dirEntry) {
              if (!dirEntry) {
              ...
              var readFile = /^text\//i.test(mimetype)
                ? "readUtf8File"
                : "readBinaryFile";
              return selectedArchive[readFile](
                dirEntry,
                function (fileDirEntry, content) {
                  var fullUrl = fileDirEntry.namespace + "/" + fileDirEntry.url;
                  if (params.assetsCache)
                    selectedArchive.cssCache.set(fullUrl, content);
                  if (/text\/css/i.test(mimetype))
                    uiUtil.replaceCSSLinkWithInlineCSS(link, content);
                  else
                    uiUtil.feedNodeWithDataURI(link, "href", content, mimetype);
                  cssFulfilled++;
            ...
            .catch(function (e) {
              console.error(
                "Could not find DirEntry for link element: " + url,
                e,
              );
              cssCount--;
  ...
  function insertMediaBlobsJQuery() {
    var iframe = iframeArticleContent.contentDocument;
    Array.prototype.slice
      .call(iframe.querySelectorAll("video, audio, source, track"))
      .forEach(function (mediaSource) {
        var source = mediaSource.getAttribute("src");
        source = source
          ? uiUtil.deriveZimUrlFromRelativeUrl(source, baseUrl)
          : null;
        ...
        if (!source || !regexpZIMUrlWithNamespace.test(source)) {
          if (source)
            console.error("No usable media source was found for: " + source);
          return;
        }
        var mediaElement = /audio|video/i.test(mediaSource.tagName)
          ? mediaSource
          : mediaSource.parentElement;
        // If the "controls" property is missing, we need to add it to ensure jQuery-only users can operate the video. See kiwix-js #760.
        if (
          /audio|video/i.test(mediaElement.tagName) &&
          !mediaElement.hasAttribute("controls")
        )
          mediaElement.setAttribute("controls", "");
        selectedArchive.getDirEntryByPath(source).then(function (dirEntry) {
          return selectedArchive.readBinaryFile(
            dirEntry,
            function (fileDirEntry, mediaArray) {
              var mimeType = mediaSource.type
                ? mediaSource.type
                : dirEntry.getMimetype();
...
function updateCacheStatus(title) {
  if (params.assetsCache && /\.css$|\.js$/i.test(title)) {
    ...
    cacheBlock.textContent =
      (translateUI.t("spinner-caching") || "Caching") + " " + title + "...";
  }
}
...
function goToArticle(path, download, contentType) {
  ...
  selectedArchive
    .getDirEntryByPath(path)
    .then(function (dirEntry) {
      var mimetype = contentType || dirEntry ? dirEntry.getMimetype() : "";
      if (dirEntry === null || dirEntry === undefined) {
        uiUtil.spinnerDisplay(false);
        uiUtil.systemAlert(
          (translateUI.t("dialog-article-notfound-message") ||
            "Article with the following URL was not found in the archive:") +
            " " +
            path,
          translateUI.t("dialog-article-notfound-title") ||
            "Error: article not found",
        );
      } else if (
        download ||
        /\/(epub|pdf|zip|.*opendocument|.*officedocument|tiff|mp4|webm|mpeg|mp3|octet-stream)\b/i.test(
          mimetype,
        )
      ) {
        download = true;
    ...
    .catch(function (e) {
      uiUtil.systemAlert(
        translateUI.t("dialog-article-readerror-message") ||
          "Error reading article with url:" + " " + path + " : " + e,
        translateUI.t("dialog-article-readerror-title") ||
  ...
  if (selectedArchive !== null && selectedArchive.isReady()) {
    ...
    selectedArchive.getRandomDirEntry(function (dirEntry) {
      if (dirEntry === null || dirEntry === undefined) {
        uiUtil.spinnerDisplay(false);
        uiUtil.systemAlert(
          translateUI.t("dialog-randomarticle-error-message") ||
            "Error finding random article",
          translateUI.t("dialog-article-notfound-title") ||
            "Error: article not found",
        );
      } else {
        ...
        if (
          selectedArchive.file.articlePtrPos ||
          dirEntry.getMimetype() === "text/html" ||
  ...
  selectedArchive.getMainPageDirEntry(function (dirEntry) {
    if (dirEntry === null || dirEntry === undefined) {
      ...
      if (
        ...
        console.error(
          "The main page of this archive does not seem to be an article",
        );
        uiUtil.spinnerDisplay(false);
        document.getElementById("welcomeText").style.display = "";
      }
