...
function(util) {
    ...
    var regexpDependency = new RegExp(/\/([I-]\/.*[^\/]+)$/);

    self.addEventListener('fetch', function(event) {
        console.log('ServiceWorker handling fetch event for : ' + event.request.url);

        // TODO handle the dummy article more properly
        if ((regexpArticle.test(event.request.url) || regexpDependency.test(event.request.url))
                && !util.endsWith(event.request.url, 'dummyArticle.html')) {
        
        console.log('Asking the backend for a content', event.request.url);
        event.respondWith(new Promise(function(resolve, reject) {
            var regexpResult;
            var titleName;
            if (regexpArticle.test(event.request.url)) {
                regexpResult = regexpArticle.exec(event.request.url);
                titleName = regexpResult[1];
                console.log("It's an article : " + titleName);
            }
            else {
                regexpResult = regexpDependency.exec(event.request.url);
                titleName = regexpResult[1]; 
                console.log("It's a dependency : " + titleName);
            }
                ...
                if (titleName === event.data.titleName && event.data.action === 'giveContent') {
                    console.log('content message received for ' + titleName, event.data);
                        var responseInit = {
                            ...
                            statusText: 'OK',
                            headers: {
                                // TODO : change content-type for dependencies
                                'Content-Type': 'text/html'
                            }
                        };

                    var responseBody = event.data.content;

                    var httpResponse = new Response(responseBody, responseInit);

                    console.log('ServiceWorker responding to the HTTP request for ' + titleName);
        ...
        // Give a sample JPEG image (Wikipedia logo) for all .jpg images
        else if (regexpJpeg.test(event.request.url)) {
            var responseInit = {
                status: 200,
                statusText: 'OK',
                headers: {
            ...
            var blob = b64toBlob(base64Image, "image/jpeg");
